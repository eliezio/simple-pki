...
@Component
public class HandleOpenSSLDateHeaderFilter implements Filter {

    @Override
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        throws IOException, ServletException {
        chain.doFilter(new HeaderMapRequestWrapper((HttpServletRequest) request), response);
    }
    ...
    private static class HeaderMapRequestWrapper extends HttpServletRequestWrapper {
        ...
        public long getDateHeader(String name) {
            ...
            return Optional.ofNullable(value)
                .flatMap(OpenSSLDateTimeParser::parse)
                .orElseGet(() -> super.getDateHeader(name));
        }
    }
}
