...
public class PkiRestController {
    ...
    public ResponseEntity<String> issueCertificate(Reader csrReader)
        ...
        return attachmentBuilder("cert.pem", APPLICATION_X_PEM_FILE)
            .header("X-Cert-Serial-Number", cert.getSerialNumber().toString())
            .body(PemConverter.toPem(cert));
    }

    @GetMapping("/certificates/{serialNumber}")
    public ResponseEntity<String> getCertificate(@PathVariable long serialNumber)
        throws Exception {
    ...
    public ResponseEntity<Void> revokeCertificate(@PathVariable long serialNumber) {
